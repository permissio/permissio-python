"""
Configuration types and utilities for the Permissio.io SDK.
"""
...
# Constants
DEFAULT_API_URL = "https://api.permissio.io"
...
@dataclass
class PermissioConfig:
    """
    Configuration for the Permissio.io SDK.

    Attributes:
        token: API key for authentication (required). Must start with 'permis_key_'.
        api_url: Base URL for the Permissio.io API. Defaults to https://api.permissio.io.
...
class ConfigBuilder:
    """
    Builder pattern for creating PermissioConfig instances.
    ...
    def build(self) -> PermissioConfig:
        """
        Build the configuration.

        Returns:
            The built PermissioConfig instance.
        """
        return PermissioConfig(
            token=self._token,
    ...
    def build_with_validation(self) -> PermissioConfig:
        """
        Build the configuration and validate it.

        Returns:
            The built and validated PermissioConfig instance.
...
def resolve_config(config: Any) -> PermissioConfig:
    """
    Resolve configuration from various input types.

    Args:
        config: A PermissioConfig instance, a dict with configuration values,
                or a string (treated as API token).

    Returns:
        A PermissioConfig instance.

    Raises:
        ValueError: If the config type is not recognized.
    """
    if isinstance(config, PermissioConfig):
        return config

    if isinstance(config, str):
        return PermissioConfig(token=config)
