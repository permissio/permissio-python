...
class BaseApiClient:
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        Raises:
            PermissioApiError: An appropriate error based on the status code.
        """
        ...
        if status_code == 401:
            raise PermissioAuthenticationError(message=message, details=details, request_id=request_id)
        elif status_code == 403:
            raise PermissioPermissionError(message=message, details=details, request_id=request_id)
        elif status_code == 404:
            raise PermissioNotFoundError(message=message, details=details, request_id=request_id)
        elif status_code == 409:
            raise PermissioConflictError(message=message, details=details, request_id=request_id)
        elif status_code == 429:
            retry_after = response.headers.get("Retry-After")
            retry_after_int = int(retry_after) if retry_after and retry_after.isdigit() else None
            raise PermissioRateLimitError(
                message=message, retry_after=retry_after_int, details=details, request_id=request_id
            )
        else:
            raise PermissioApiError(
    ...
    def _should_retry(self, exception: Exception, attempt: int) -> bool:
        ...
        if isinstance(exception, (PermissioNetworkError, PermissioTimeoutError)):
            return True

        # Retry on specific API errors
        if isinstance(exception, PermissioApiError) and exception.is_retryable:
            return True
    ...
    def _calculate_retry_delay(self, attempt: int, exception: Optional[Exception] = None) -> float:
        ...
        if isinstance(exception, PermissioRateLimitError) and exception.retry_after:
    ...
    def request(
        ...
        *,
        ...
        Raises:
            PermissioApiError: If the request fails after all retries.
        """
        ...
        for attempt in range(self.config.retry_attempts + 1):
            try:
                ...
                last_exception = PermissioTimeoutError(original_error=e)
            except httpx.NetworkError as e:
                last_exception = PermissioNetworkError(f"Network error: {e}", original_error=e)
            except (PermissioApiError, PermissioNetworkError, PermissioTimeoutError) as e:
                last_exception = e
        ...
        if last_exception:
            raise last_exception
        raise PermissioNetworkError("Request failed with unknown error")
